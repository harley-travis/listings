...
//        }
	...
	public static function get_band_category(){
		$db = Database::getDB();
		$query = 'SELECT band_categories.band_category, band.band_id
			 	  FROM band_categories
				  INNER JOIN band
				  ON band_categories.band_categories_id=band.band_id
				  ORDER BY band_categories.band_category';
		$statement = $db->prepare($query);
		$statement->execute();
		$band_categories = $statement->fetchAll();
		$statement->closeCursor();
